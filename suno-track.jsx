...
function buildChecklist(data) {
  ...
  return [
    ...
    { label: "Meta-tags in lyrics", ok: tagCount >= 3, note: `${tagCount} tags` },
  ];
}
...
// COPY BUTTON
// ============================================================
function CopyButton({ text, label, accent }) {
...
// BPM DISPLAY
// ============================================================
function BPMDisplay({ bpm, accent, dark }) {
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
      ...
      />
      <div>
        <span style={{ fontSize: "22px", fontWeight: "700", color: accent, fontFamily: "monospace", letterSpacing: "0.05em" }}>
          {bpm}
        </span>
        <span style={{ fontSize: "10px", color: dark ? "#555" : "#aaa", marginLeft: "4px", letterSpacing: "0.15em", textTransform: "uppercase" }}>
          BPM
        </span>
      </div>
    </div>
...
// COLLAPSIBLE SECTION
// ============================================================
function CollapsibleSection({ title, children, accent, dark, defaultOpen = true, animClass = "" }) {
  const [open, setOpen] = useState(defaultOpen);
  const textDim = dark ? "#aaa" : "#44415a";

  return (
    <div className={`suno-animate ${animClass}`} style={{ marginBottom: "24px" }}>
      <button
...
function GenerationSettings({ settings, accent, dark }) {
  ...
  const ToggleRow = ({ label, value, options }) => (
    <div style={{ ...rowStyle, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
      <span style={{ fontSize: "13px", color: textMain }}>{label}</span>
  ...
  const SliderRow = ({ label, value }) => (
    <div style={{ ...rowStyle }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: "14px" }}>
        <span style={{ fontSize: "13px", color: textMain }}>{label}</span>
        <span style={{ fontSize: "12px", fontFamily: "monospace", color: accent, letterSpacing: "0.05em" }}>
          {value}%
        </span>
      </div>
      <div style={{ position: "relative", height: "28px", display: "flex", alignItems: "center" }}>
        {Array.from({ length: NUM_TICKS + 1 }).map((_, i) => (
          <div key={i} style={{
            position: "absolute",
...
// ============================================================
function Checklist({ data, accent, dark, animClass = "" }) {
  ...
  return (
    ...
    >
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "12px" }}>
        ...
        <div style={{ fontSize: "10px", color: allGood ? accent : "#ff6644", fontFamily: "monospace", letterSpacing: "0.05em" }}>
          {passCount}/{items.length} {allGood ? "✓" : ""}
        </div>
      </div>
      <div style={{ height: "3px", background: dark ? "#14142a" : "#e0e0d8", borderRadius: "2px", marginBottom: "14px", overflow: "hidden" }}>
        <div style={{
          height: "100%",
          width: `${pct}%`,
      ...
      {items.map((item, i) => (
        <div key={i} style={{ display: "flex", alignItems: "center", gap: "10px", marginBottom: i < items.length - 1 ? "8px" : 0 }}>
          <div style={{
            width: "15px",
            height: "15px",
            borderRadius: "50%",
          ...
          <div style={{ flex: 1, display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: "4px" }}>
            <span style={{ fontSize: "11px", color: dark ? "#5a5580" : "#8880a0" }}>{item.label}</span>
            <span style={{ fontSize: "10px", color: item.ok ? accent + "88" : "#ff664488", fontFamily: "monospace" }}>{item.note}</span>
          </div>
        </div>
      ))}
    </div>
...
function SectionNavigator({ lyrics, accent, dark, animClass = "" }) {
  ...
  lyrics.split("\n").forEach((line, i) => {
    if (line.startsWith("[") && !line.startsWith("([")) {
      ...
      if (keys.some(k => label.startsWith(k)) && !sections.find(s => s.label === label)) {
        sections.push({ label, id: `section-${i}` });
      }
    }
  });
  ...
  return (
    ...
    >
      <div style={{ width: "100%", fontSize: "11px", color: textDim, letterSpacing: "0.1em", textTransform: "uppercase", marginBottom: "6px" }}>
        Jump to section
      </div>
      {sections.map((s, i) => (
        <button
      ...
      >
        ...
        >
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: "14px" }}>
            <div>
              <div style={{ fontSize: "11px", color: accent, letterSpacing: "0.12em", textTransform: "uppercase", marginBottom: "10px", opacity: 0.85 }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: "14px" }}>
              <button
                ...
                onMouseEnter={e => { e.currentTarget.style.borderColor = accent + "55"; e.currentTarget.style.color = accent + "99"; }}
                onMouseLeave={e => { e.currentTarget.style.borderColor = cardBorder; e.currentTarget.style.color = textDim; }}
              >
                {dark ? "☀ Light" : "☾ Dark"}
              </button>
        ...
        {/* LYRICS */}
        <div className="suno-animate suno-animate-4" style={{ marginBottom: "28px" }}>
...
window.renderSunoTrack = function(trackData, container) {
  ...
  root.render(React.createElement(SunoTrack, { trackData }));
};
